...
PoC 環境で見落としがちなのがコストです。VM は停止しない限り課金が続きます。このテンプレートでは DevTestLab の自動停止スケジュールを組み込んでおり、指定時刻（デフォルト 19:00 JST）に VM が自動停止します。自動起動は含まれていないので、翌朝の起動はポータルか CLI で手動実行する運用です。
