...
PoC 環境で見落としがちなのがコストです。VM は停止しない限り課金が続きます。このテンプレートでは DevTestLab の自動停止スケジュールを組み込んでおり、指定時刻（デフォルト 19:00 JST）に VM が自動停止します。夜間や週末の無駄な課金を防げます。自動起動は対応していないので、翌朝は Azure Portal か `az vm start` で手動起動する運用です。
