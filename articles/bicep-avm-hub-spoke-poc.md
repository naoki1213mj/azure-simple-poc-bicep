...
AVM を使ってみた率直な感想としては、diagnosticSettings や Private Endpoint がモジュール内に統合されているのが機概でした。リソースごとにバラバラに書く必要がなく、パラメータ 1 つで有効化できます。一方で、各モジュールの README を読んでパラメータ体系を覆えるまでは調べものの比率が高かったです。「お作法を覆えれば楽」で、そのお作法を覆えるまでがやや大変。ただ、一度パターンを掴むと似たリソースの追加はかなり速くなります。
