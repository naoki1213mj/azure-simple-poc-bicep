...
以前は ALZ-Bicep や CARML といったモジュール集がありましたが、2026 年 2 月にアーカイブされ、AVM が後継になりました。自分で書くと数十行になるリソース定義が、AVM モジュールを使うとパラメータを渡すだけで済みます。たとえば NSG の定義はこんな感じです。
...
├── AzureBastionSubnet  (/26) … SSH/RDP の踏み台
├── snet-agw            (/24) … Application Gateway + WAF v2（オプション）
└── Private DNS Zones         … PE 用の名前解決

Spoke VNet (10.1.0.0/16)
├── snet-vm             (/24) … CPU/GPU VM + NAT Gateway
└── snet-pep            (/24) … Private Endpoint（Blob, AI Services, Key Vault）

共有サービス
├── Log Analytics              … 診断ログ集約
├── Key Vault                  … シークレット管理
├── Storage Account            … データ保存
├── AI Services                … GPT モデル（オプション）
└── Recovery Services Vault    … バックアップ（オプション）
```
...
最初、このルールを自分で調べて書いていたのですが、ルールが 1 つ足りなくてデプロイ後に Bastion が接続できず焦りました。公式の「Bastion NSG の要件」ページを見ながら書くのが確実です。
...
| DR | `avm/res/recovery-services/vault` | 0.12.0 |
...
AVM モジュールはセマンティックバージョニングに従っていますが、マイナーバージョンアップでもパラメータ名が変わることがあります。`0.4.x` → `0.5.x` で `subnetAddressPrefix` が `addressPrefix` になった、という変更に遭遇しました。バージョンを上げるときは CHANGELOG を読む癖をつけたほうがよさそうです。
