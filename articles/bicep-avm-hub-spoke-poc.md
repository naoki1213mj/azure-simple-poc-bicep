...
GitHub Actions で CI/CD も組んでいます。

CI（PR 時）は 4 つのチェックを走らせます。Bicep Lint で構文エラーを検出し、PSRule for Azure で WAF/CAF 準拠を自動チェックします。Gitleaks で機密情報（API キーや接続文字列）の混入を検知し、What-If の結果を PR コメントに自動投稿して変更差分をレビューできるようにしています。

CD はブランチ戦略で環境を分けています。main への push で dev 環境に自動デプロイし、prod 環境は手動トリガー + GitHub Environment の承認ゲートで保護しています。OIDC 認証でシークレットを GitHub に保存しない構成です。

「PR を出したら自動でセキュリティチェックが走り、マージしたら dev にデプロイされる」という Shift-Left な流れを、Bicep プロジェクトでも実現できています。

### 付属ドキュメント

テンプレート本体のほかに、運用で必要になるドキュメントも用意しています。

- デプロイ手順書（azd / Azure CLI）
- 環境削除手順書（依存関係を考慮した削除順序）
- SSL 証明書の発行・更新手順（自己署名 / Let's Encrypt / 商用 CA）
- VM リモート接続ガイド（SSH トンネリング、Jupyter、marimo）
- ログ収集一覧、コスト見積もりガイド

「テンプレートをデプロイしたあとどうするか」まで含めてリポジトリに入れてあるので、引き継ぎや新メンバーのオンボーディングにも使えます。
