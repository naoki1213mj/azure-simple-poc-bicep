...
| 学習コスト | ARM スキーマを理解すれば書ける | AVM のパラメータ体系を覚える必要がある |
...
想定している用途は「社内の開発チームが自由に使える閉じた検証環境」です。いくつかのユースケースを念頭に置いています。

- GPU VM で LLM の推論やファインチューニングを動かす
- CPU VM で Streamlit や marimo のデータ分析アプリをホストし、Application Gateway + WAF v2 経由で社内に公開する
- AI Services 経由で GPT モデルの API を Private Endpoint 越しに呼ぶ
- 分析用のデータを Storage Account に置いて、VM からマウントして処理する

VM ベースの構成にしたのは自由度が理由です。App Service や Container Apps だとランタイムやポートの制約がありますが、VM なら CUDA ドライバを入れるのも、Podman でコンテナを動かすのも、好きなポートでサービスを立てるのも自由にできます。分析用途だと「ちょっと試したい」が多いので、この自由さは重要でした。

外部からの直接アクセスは遮断しつつ、Bastion 経由の SSH でメンテナンスできる構成です。既存の社内 NW とはつながないので、Azure Firewall のような大掛かりなものは不要で、NSG と Private Endpoint で十分だろうと判断しました。
...
### Bastion 経由の SSH トンネリング

VM にパブリック IP がないため、ローカル PC から直接 SSH することはできません。Azure Bastion の Standard SKU にはネイティブクライアント接続機能があり、`az network bastion tunnel` でローカルポートと VM の SSH ポートをトンネル接続できます。

```bash
# ローカルの 50022 番ポートを VM の 22 番にトンネル
az network bastion tunnel \
  --name bas-dev0001-japaneast-001 \
  --resource-group rg-hub-dev0001-japaneast-001 \
  --target-resource-id "<VM のリソース ID>" \
  --resource-port 22 --port 50022

# 別ターミナルで SSH 接続
ssh -p 50022 -i infra/keys/id_rsa azureuser@127.0.0.1
```

このトンネルを使えば、VS Code の Remote-SSH でも VM に接続できます。Jupyter や Streamlit をリモートで動かしたいとき、`-L` オプションでポートフォワーディングを重ねれば、ローカルブラウザからアクセスすることも可能です。Azure Portal の Bastion 画面からブラウザ SSH する方法もありますが、ターミナルで作業したい人にはトンネル接続のほうが使い勝手がよいです。
...
VM には Azure Monitor Agent（AMA）をインストールし、Data Collection Rule で CPU・Memory・Disk のパフォーマンスカウンターと Syslog を収集しています。収集したメトリクスに対して Scheduled Query Rules でアラートを設定し、CPU 90% 超、メモリ 90% 超、ディスク残量 15% 未満の場合に Action Group 経由でメール通知が飛びます。

サブスクリプションの Activity Log も Log Analytics に転送しています。誰がいつ何をしたかの監査ログが自動的に蓄積されるので、セキュリティインシデント時の調査に使えます。
...
### パラメータと cloud-init による自動化

1 つのテンプレートで dev と prod を切り替えられる設計にしています。`vmPattern` で CPU のみ・GPU のみ・両方を選べて、`enableAppGateway` や `enableBackup` のフラグで機能単位でオン・オフできます。dev は CPU VM 1 台の最小構成、prod は GPU VM + AppGW + Backup + Defender のフル構成。同じコードベースから両方出せるのは IaC ならではの強みです。

VM がデプロイされると、cloud-init でデータディスクのパーティション作成・マウントと OS ディスクの LVM 拡張が自動実行されます。GPU VM なら CUDA ドライバや NVIDIA Container Toolkit のインストールも含まれるので、デプロイ完了後すぐに `nvidia-smi` が通る状態になります。

PoC 環境で見落としがちなのがコストです。VM は停止しない限り課金が続きます。このテンプレートでは DevTestLab の自動停止スケジュールと Azure Automation の自動起動を組み込んでいます。停止はデフォルト 19:00 JST、起動は 9:00 JST で、夜間・週末の無駄な課金を防げます。

### azd ・ CI/CD ・運用ドキュメント

Azure Developer CLI（`azd`）に対応しているので、`azd up` 一発でデプロイできます。`preprovision.sh` で SSH 鍵の生成やリソースプロバイダーの登録を自動処理し、`postprovision.sh` で VNet Flow Log の作成や SSL 証明書の Key Vault 登録を行います。手動デプロイだと 5 ステップ必要な事前準備が、azd ではフックに吸収されます。

GitHub Actions で CI/CD も組んでいます。CI（PR 時）は Bicep Lint、PSRule for Azure（WAF/CAF 準拠）、Gitleaks（機密情報検知）、What-If の PR コメント投稿の 4 つ。CD は main push で dev 自動デプロイ、prod は承認ゲート付き手動トリガーで、OIDC 認証によりシークレットを GitHub に保存しない構成です。

テンプレート本体のほかに、運用ドキュメントも用意しています。デプロイ手順、環境削除、SSL 証明書の発行・更新、VM リモート接続ガイド、ログ収集一覧、コスト見積もりガイドなど、「デプロイしたあとどうするか」まで含めてリポジトリに入れてあります。
...
**Bastion のサブネット名** — `AzureBastionSubnet` は Azure 側の要件で固定です。`snet-bastion` にしたくなりますが、Bastion が動きません。

**PE + DNS の 3 点セット** — PE を作っただけでは名前解決できません。`privateDnsZoneGroup` で DNS Zone と紐づけ、その Zone を Hub と Spoke 両方の VNet にリンクする必要があります。片方でも欠けると `nslookup` で Private IP が返りません。

**Peering の双方向** — Hub→Spoke だけ設定して「つながった」と思ったら、戻り通信が通りませんでした。別 RG の場合はモジュール分割 + スコープ切り替えも必要です。

**NAT Gateway の Public IP** — NAT Gateway を作っても Public IP を関連付けないとアウトバウンド通信ができません。AVM の `publicIPAddressObjects` で自動生成できます。

### AVM ・ Bicep 系

**NSG ルールの properties ネスト** — AVM NSG v0.5.0 では `{ name: ..., properties: { ... } }` 形式が必須です。フラットに書くとビルドエラーになります。

**バージョンの破壊的変更** — `0.4.x` → `0.5.x` でパラメータ名が変わることがあります。Registry に存在しないバージョンを指定すると `BCP192` エラーです。

**customData の二重 base64** — AVM VM の `customData` は生テキストで渡します。`base64()` をかけると二重エンコードになり、cloud-init が走りません。ディスクが未マウントの原因を調べたら、そもそも cloud-init が実行されていなかったというオチでした。

**EncryptionAtHost 機能登録** — v6 世代 VM を使う場合、`Microsoft.Compute/EncryptionAtHost` をサブスクリプションで事前に登録する必要があります。

## おわりに

Bicep + AVM で Hub-Spoke の PoC 環境を組んでみました。ネットワーク、VM、セキュリティ、AI サービスまで一通りのリソースを扱えたので、Bicep の勉強としてはちょうどよい規模感だったと思います。

AVM を使ってみた率直な感想としては、diagnosticSettings や Private Endpoint がモジュール内に統合されているのが便利でした。リソースごとにバラバラに書く必要がなく、パラメータ 1 つで有効化できます。一方で、各モジュールの README を読んでパラメータ体系を覚えるまでは調べものの比率が高かったです。「お作法を覚えれば楽」で、そのお作法を覚えるまでがやや大変。ただ、一度パターンを掴むと似たリソースの追加はかなり速くなります。

Application Gateway は AVM を使わず `resource` で直書きしました。AVM の Application Gateway モジュールはパラメータが多く、WAF Policy との組み合わせ方が独特で、直書きのほうが見通しがよかったためです。「AVM にあるからといって必ず使わなければいけないわけではない」というのも実感です。

次は Microsoft Foundry の Project 設定や、Spoke を追加してマルチチーム構成にするあたりを掘り下げたいです。Application Gateway の HTTPS 化については、SSL 証明書の発行から Key Vault 登録、AppGW リスナーへの適用まで手順をリポジトリの docs に用意しています。
