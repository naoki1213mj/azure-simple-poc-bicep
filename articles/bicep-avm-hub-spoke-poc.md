...
PoC 環境で見落としがちなのがコストです。VM は停止しない限り課金が続きます。このテンプレートでは DevTestLab の自動停止スケジュールを組み込んでおり、指定時刻（デフォルト 19:00 JST）に VM が自動停止します。自動起動については旧テンプレートでは Azure Automation の Runbook で実現していましたが、AVM 版では DevTestLab の停止のみに絞っています。起動は手動か、必要に応じて Automation を別途追加する想定です。
