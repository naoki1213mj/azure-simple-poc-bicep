...
OS は RHEL 9.4、SSH 鍵認証のみ。cloud-init でデータディスクのフォーマット、OS ディスク拡張、開発ツールのインストールを自動実行します。
...
| CUDA Toolkit | 12.4 |
| NVIDIA Container Toolkit | 最新安定版（公式リポ） |
| Podman | RHEL 9.4 標準版 |

GPU VM は `/home` を LLM モデル格納を想定して 1.0 TB に拡張します。
...
| /home | 1.0 TB |
| /var | 30 GB |
| /datadrive | パラメータ指定（デフォルト 1536 GB） |
